"""Build bucketed attribution matrix report for a persisted run."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path, markdown_path = write_attribution_matrix(report, output_dir=args.output_dir)
    payload = {
        ...
        "artifacts": {
            "attribution_matrix_json_path": str(json_path),
            "attribution_matrix_chinese_markdown_path": str(markdown_path),
        },
    }
    if args.print_markdown:
        print(render_attribution_matrix_markdown_zh(report))
    else:
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build bucketed attribution matrix report for a persisted run")
