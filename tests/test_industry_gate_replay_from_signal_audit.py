...
def test_industry_gate_replay_comparison_builds_from_signal_audit(tmp_path: Path) -> None:
    source_dir, industry_dir, baseline_dir = _write_fixture(tmp_path)
    ...
    assert report["comparison"]["tested_candidate_id"] == "industry_very_strong"
...
def test_industry_gate_replay_comparison_writes_outputs(tmp_path: Path) -> None:
    ...
    assert "含行业保守档 Runner Replay 对照" in markdown
...
def _write_fixture(root: Path) -> tuple[Path, Path, Path]:
    ...
    for path in (source_dir, industry_dir, baseline_dir):
        path.mkdir()
    ...
    return source_dir, industry_dir, baseline_dir
