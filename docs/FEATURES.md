...
已有 full `signal_audit.parquet` 或历史 `signal_audit.json`、对应 `run_plan.json` 和股票池文件时，可先构建 Strategy Decision Event Table：

```bash
att-scored-entry-allocation-tuning --build-decision-event-table \
  --signal-audit reports/baoma-v1-full-source/signal_audit.parquet \
  --run-plan reports/baoma-v1-full-source/run_plan.json \
  --progress-log reports/scored-entry-allocation-tuning/decision-event-table-progress.ndjson \
  --progress-interval-rows 100000 \
...
att-scored-entry-allocation-tuning --mode smoke --run-full-study \
  ...
  --progress-log reports/scored-entry-allocation-tuning/full-study-progress.ndjson \
  --progress-interval-trials 10 \
  --output-dir reports/scored-entry-allocation-tuning
```
...
att-segmented-factor-contribution-matrix \
  ...
  --output-dir reports/segmented-factor-contribution-matrix-baoma-v1-fixed-sample-2015-2024-maxhold800
```

该命令默认按入场日期切分：

- `2015_2016_high_volatility_bear_shock`：2015-01-01 到 2016-12-31
- `2017_2018_structural_to_bear`：2017-01-01 到 2018-12-31
- `2019_2020_recovery_structural_bull`：2019-01-01 到 2020-12-31
...
- Fixed-parameter Scored Portfolio smoke run：可直接消费 Strategy Decision Event Table，使用固定 scorer 与 portfolio controls 产出 selected entries、cash movements、position snapshots、equity curve、blocked entries、funnel 和 metrics；适合验证固定 `minimum_score` 与固定组合控制下的真实候选竞争结果。
...
- Data Preflight 性能画像与离线并行：`DataPreflightReport.performance_profile` 和 progress completed 事件会记录 symbol 准备阶段的 `load_bars` / `load_indicators` / `load_tradability` / `build_indicator_frame` / `slice_and_assess_quality` 聚合耗时、调用次数、平均耗时和 symbol 阶段 wall time；离线只读、无 provider 的 preflight 会自动最多 4 worker 并行读取 symbol 快照，联网或刷新快照场景保持串行。
- RunPlan 股票池 prepared symbol 复用：同一次执行中，股票池 preflight 成功准备出的 `PreparedSymbolData` 会在过滤后传给正式 prepared data；仅当 symbol / asset_type / adjustment 匹配时复用，不跨 RunPlan 落盘复用，不改变 preflight 错误隔离。
- Baoma 归因构建复用：entry attribution context 在每只股票内预计算 `date -> DEA 水线年龄` 和 completed weekly KDJ/MACD evidence，避免 symbol evidence 构建时每根 K 线重复排序、回扫 MACD 序列和扫描 weekly 指标日期；只减少计算，不改变归因字段或筛选口径。
- Prepared Run Data 性能画像：prepared data 完成后会写出 `prepare_run_data_profile` progress 事件，记录股票/benchmark/行业引用数量、snapshot provenance 动作统计、source snapshot 引用次数、provider fetch 次数、warmup incomplete 次数、指标快照复用/构建统计、prepared symbol 复用数量，以及 symbol 准备阶段的 `load_bars` / `slice_and_assess_quality` / `load_indicators` / `build_indicator_frame` / `load_tradability` 聚合耗时；用于先定位重复读取和重复构建热点，不改变回测结果。
...
### 5.1 MyStrategy 入场条件（必要条件 + 可选过滤）

**必要条件（6 条同时成立）：**

1. `close < prev_close`（收阴）
2. `close > MA60`（趋势之上）
3. `(close - MA60) / MA60 ≥ min_ma60_dist_pct`（与 MA60 最小距离，默认 5%）
4. `DEA > 0`（处于多头宏观区）
5. 过去 `dea_lookback_days` 内出现过 `DEA < 0`（刚刚翻多）
6. 容量检查：当前持仓数 < `max_positions`

**可选过滤（默认关闭，config 中 true/false 控制）：**

- `require_stock_ma_bull`：要求 MA25 > MA60（个股均线多头排列）
- `require_week_macd_above_zero`：要求周线 MACD 不在区间0（周线 MACD 柱为正）
- `require_month_macd_above_zero`：要求月线 MACD 不在区间0（月线 MACD 柱为正）

> 区间0 = 周/月 MACD 柱 ≤ 0（空头区）；区间1-3 = MACD 柱 > 0 的各阶段。
6. 当前未持仓或满足加仓条件
...
| `min_ma60_dist_pct` | 入场最小 MA60 距离（默认 0.05 = 5%），过滤紧贴均线的假突破 |
| `max_add_count` | 每笔最多加仓次数（默认 0 = 禁止加仓） |
| `require_stock_ma_bull` | true = 要求 MA25 > MA60 才入场（默认 false） |
| `require_week_macd_above_zero` | true = 要求周线 MACD 不在区间0（默认 false） |
| `require_month_macd_above_zero` | true = 要求月线 MACD 不在区间0（默认 false） |
