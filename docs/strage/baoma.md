...
|---|---|
| 总资产 | 100 亿 |
| 最大持仓数量 | 800，既是每股最大市值分母，也是同时持仓股票数上限 |
| 每个股票最大市值 | 总资产 / 最大持仓数量 = 1250 万 |
| 单次买入目标市值 | 每个股票最大市值 * 33% = 412.5 万 |
| 仓位不足阻止买入 | 不因现金或组合仓位不足阻止买入；若同时持仓股票数达到 800，则新开仓被阻止并记录 |
| 已有持仓加仓 | 即使同时持仓数已达到 800，已有持仓仍允许按规则加仓 |
...
| `G-05` | 归因 reference 准备入口 | 已新增 `att-prepare-attribution-reference` 第一版，从已准备好的全 A 日频基础表生成 `metadata.json`、`reference_values.parquet` 或按交易日分片的 `reference_values/`，小样本可额外生成 `reference.json`；Tushare 大规模拉取后续接入该入口前置数据源 | 全量数据准备使用 `--parquet-only --partition-by-trade-date`，避免巨大 JSON 和单文件长表。 |
...
max_holding_count=800 归因重跑记录：

| 项目 | 内容 |
|---|---|
| 回测 run | `reports/baoma-v1-fixed-sample-2023-2024-maxhold800`；`max_holding_count=800`；已平仓交易 4610 笔；未平仓 121 笔；执行拒单 0。 |
| reference 快照 | `data/snapshots/attribution_reference/full_a_main_chinext_star/2022-08-26_2024-12-31_baoma-v1-maxhold800-entry-scope-full-a-industry-backfilled` |
| reference 口径 | `--reference-fetch-scope all --emit-run-entry-scope`；4610 笔交易对应 9220 个 symbol/date 入场反查点；reference rows 350360；异常 88。 |
| wide samples | `reports/baoma-v1-fixed-sample-2023-2024-maxhold800/full_entry_scope_environment_fit_review/attribution_wide_samples.json`；4610 笔 completed trades；字段索引 176 个。 |
| environment_fit | 默认字段 31 个；单因子分组 160 个；白名单二因子组合 798 个。 |
| 报告入口 | `reports/baoma-v1-fixed-sample-2023-2024-maxhold800/full_entry_scope_environment_fit_review/environment_attribution_review.zh.md`、`environment_fit.enriched.zh.md`、`strategy_environment_profile.zh.md` |
| 数据质量 | 旧 reference 对 800 持仓样本曾出现 1611 笔 `reference_record_missing`；已按 maxhold800 entry scope 重建 reference 后重跑，申万一级行业缺失降为 0；`PE_TTM` 缺失 376 笔按 missing 保留。 |
| 手工校验样本 | `manual_feature_validation_samples/entry_factor_bucket_trade_samples.zh.md`；已补齐 `trades.json.closed_trades[].trade_index`，并将异常拆为 `trade_exception_codes`、`environment_default_exception_codes`、`entry_factor_exception_codes`、`audit_only_exception_codes`。 |
...
34. `att-prepare-attribution-reference` 显式接收 `--start-date` 和 `--end-date`；日期范围需要覆盖 run 的 warmup 起点到回测结束日期，以支持 ATR、20/60 日收益波动率、近 20/60 日高低点等窗口指标。需要 warmup 但不输出 warmup rows 时使用 `--emit-start-date`。归因宽表只对实际入场信号日输出样本，warmup 区间仅用于计算参考指标。
...
36. 全 A 参考快照中表格数据使用 Parquet，metadata/provenance/参数/字段版本/异常统计使用 JSON；正式大样本使用 `--parquet-only --partition-by-trade-date`，`reference.json` 只保留给小样本调试。
...
| 单股满仓目标 | 每个股票最大市值 = 总资产 / 最大持仓数量；当前版为 1250 万 |
...
| 18 | 同一天多个无持仓股票同时满足入场条件时没有固定成交口径 | 如果隐含排序或只买部分股票，会把第一版样本回测变成选股排序优化 | 已确认全部买入，直到达到 800 个不同股票持仓上限；超过上限记录 `MAX_HOLDING_COUNT` |
...
11. 新增 RunPlan 样本，固定股票列表、回测区间、初始资金 100 亿、最大持仓数量 800、费用、滑点和 A 股约束；费用、税费、滑点必须来自配置项。
...
- 第一版是 `Trade-Sample Backtest`：总资产 100 亿，当前最大持仓数量 800，每个股票最大市值 1250 万，单次买入目标市值 412.5 万；不看总盈利，只统计成交股票和交易样本。
- 最大持仓数量 800 既是每股最大市值分母，也是同时持仓股票数硬上限；如果触发上限，新开仓不成交并记录 `MAX_HOLDING_COUNT`。
- 当前同时持仓数达到 800 时，已有持仓仍允许加仓，因为加仓不增加不同股票持仓数量。
...
- 同一天多个无持仓股票满足入场条件时，全部买入，直到达到 800 个不同股票持仓上限；第一版不做排序、不做信号强弱筛选。
...
- 同一天入场候选超过剩余持仓名额时，第一版默认按固定股票池清单顺序买入，直到达到 800 个持仓上限；RunPlan 预留排序配置，但默认不开启，后续如开启排序必须作为单独实验口径。
